...
def run_selfplay_match(
    ...
    if config.selfplay.profile_selfplay:
        external_env["TRIBES_RL_PROFILE"] = "1"
    if external_env:
    ...
    try:
        with stdout_path.open("w", encoding="utf-8") as stdout_handle, stderr_path.open("w", encoding="utf-8") as stderr_handle:
            ...
            while process.poll() is None:
                ...
                if elapsed > timeout_seconds:
                    ...
                    if bot_tail:
                        stderr += f"\n[tribes_rl] external bot stderr tails:\n{bot_tail}"
                    return subprocess.CompletedProcess(
                        args=command,
                        returncode=124,
                        stdout=stdout,
                        stderr=stderr + f"\n[tribes_rl] {label} timed out after {timeout_seconds}s",
                    )
        ...
        if config.selfplay.profile_selfplay:
            ...
            if bot_logs:
                stderr += f"\n[tribes_rl] external bot stderr logs:\n{bot_logs}"
