...
def choose_action(message: Dict[str, Any]) -> Dict[str, Optional[str]]:
    ...
    # Be conservative about plain moves. The external protocol gives us only the current
    # legal action list, not a simulator, so non-move tactical actions are usually safer
    # whenever they are close in value. This reduces flaky movement choices late in turns.
    if chosen.get("type") == "MOVE":
        best_non_move: Optional[Tuple[float, Dict[str, Any]]] = None
        for score, action in scored:
            if action.get("type") != "MOVE":
                best_non_move = (score, action)
                break

        move_bonus = direct_move_capture_bonus(view.tile(int(chosen["destination"]["x"]), int(chosen["destination"]["y"])), view)
        if best_non_move is not None:
            non_move_score, non_move_action = best_non_move
            if move_bonus < 4.5 and non_move_score >= chosen_score - 0.9:
                chosen_score, chosen = non_move_score, non_move_action
