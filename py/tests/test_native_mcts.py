...
class NativeMCTSTest(unittest.TestCase):
    ...
    def test_static_eval_experimental_keeps_baseline_priors_with_research_context(self) -> None:
        extension = load_native_mcts_extension()
        self.assertIsNotNone(extension)
        message = _message_with_military_research_context(
            ["ARCHERY", "STRATEGY", "SMITHERY", "MATHEMATICS", "CHIVALRY", "ROADS"],
            stars=24,
        )
        enemy = message["observation"]["units"][1]
        enemy.update({"type": "KNIGHT", "attack": 3.5, "movement": 3, "x": 2, "y": 1})
        message["observation"]["board"]["tiles"][1][2]["unit_id"] = enemy["id"]
    ...
    def test_java_parity_milestone4_village_capture_matches_java_city_state(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_milestone4_s19_end_turn_hidden_enemy_actions(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_milestone4_build_road_regenerates_connected_actions(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_tower_unlock_road_keeps_build_actions(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_tower_unlock_build_forces_level_up_actions(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_superunit_attack_end_turn_action_count(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_milestone4_convert_does_not_add_extra_unit_id(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_milestone4_upgrade_rammer_does_not_add_extra_unit_id(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
    ...
    def test_java_parity_milestone4_upgrade_scout_scores_unit_value(self) -> None:
        status = run_parity(
            parse_args(
                [
                    "--fixture",
