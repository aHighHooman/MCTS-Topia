...
from .mcts import (
    ...
    _bootstrap_leaf_evaluation,
    _leaf_requires_bootstrap,
)


def _uniform_priors(action_count: int) -> List[float]:
    return [1.0 / action_count] * action_count if action_count else []
...
def _coerce_static_evaluation(raw: Dict[str, Any], action_count: int) -> _Evaluation:
    ...
    if len(priors) != action_count:
        message = f"Static eval prior/action mismatch: {len(raw_priors)} priors for {action_count} actions"
        if os.getenv("TRIBES_STATIC_EVAL_STRICT", "0") == "1":
            raise ValueError(message)
        warnings.warn(message + "; falling back to uniform priors", RuntimeWarning, stacklevel=2)
        priors = _uniform_priors(action_count)
...
def run_native_static_mcts(
    ...
    while (deadline is not None and time.perf_counter() < deadline) or (deadline is None and simulations_remaining > 0):
        ...
        if evals_only_batches is not None:
            ...
            raw_selections = tree.select_leaf_batch_evals_only(frontier, max_depth, float(search_cfg.c_puct))
        ...
        for raw_selection in raw_selections:
            ...
            if cached is None:
                ...
                if eval_index < 0:
                    ...
                    if _leaf_requires_bootstrap(leaf_payload, root_payload, search_cfg):
                        cached = _bootstrap_leaf_evaluation(leaf_payload, search_cfg)
                        eval_cache[eval_key] = cached
                    else:
    ...
    if selected_paths <= 0:
        ...
        visit_distribution = {
            ...
            for action_id, prob in tree.root_visit_distribution(search_cfg.root_temperature).items()
        }
