...
public class City extends Actor{
    ...
    //Whether the city was infiltrated and should lose its next city-income step.
    private boolean infiltrated;
    ...
    public City(JSONObject obj, int cityID){
        ...
        infiltrated = obj.has("infiltrated") && obj.getBoolean("infiltrated");
    ...
    void updateBuildingEffects(GameState gameState, Building building, boolean negative, boolean onlyMatching)
    {
        ...
        switch (building.type) {
            ...
            case GRAND_BAZAR:
                ...
                addPointsWorth(TribesConfig.MONUMENT_POINTS * multiplier);
                break;
        }
    }
    ...
    /**
     * Gets the production associated to this city.
     * Negative population reduces income one-for-one, but city production never goes below zero.
     * @return the production of this city
     */
    public int getProduction(){
        int capitalBonus = isCapital ? TribesConfig.PROD_CAPITAL_BONUS : 0;
        return Math.max(0, level + production + capitalBonus + Math.min(population, 0));
    }
    ...
    public void removeUnit(int id){
        ...
        throw new IllegalStateException("Unit " + id + " does not belong to city " + actorId + ".");
    }
    ...
    public Building getEmbassy(int ownerTribeId) {
        for (Building building : buildings) {
            if (building.type == Types.BUILDING.EMBASSY &&
                    building.getOwnerTribeId(this.tribeId) == ownerTribeId) {
                return building;
            }
        }
        return null;
    ...
    public City copy(boolean hideInfo){
        ...
        c.bound = bound;
        c.actorId = actorId;
        c.pointsWorth = pointsWorth;
        c.infiltrated = infiltrated;
    ...
    public void setInfiltrated(boolean infiltrated) {
