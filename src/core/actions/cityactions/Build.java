...
public class Build extends CityAction
{
    ...
    private boolean isBuildable(final GameState gs, int cost, boolean checkIfUnique) {
        ...
        // City improvements can be built anywhere in explored city territory.
        if (!tribe.isExplored(targetPos.x, targetPos.y)) {
            return false;
        }
        if (board.getCityIdAt(targetPos.x, targetPos.y) != this.cityId) {
            return false;
        }
        if (board.getBuildingAt(targetPos.x, targetPos.y) != null) {
            return false;
        }
        ...
        if(!UnlockRules.isBuildingUnlocked(tribe, buildingType)) { return false; }
        ...
        if (buildingType == Types.BUILDING.MARKET) {
            boolean adjFound = false;
            for (Vector2d adjPos : targetPos.neighborhood(1, 0, board.getSize())) {
                Types.BUILDING adjacent = board.getBuildingAt(adjPos.x, adjPos.y);
                if (adjacent == Types.BUILDING.SAWMILL ||
                        adjacent == Types.BUILDING.WINDMILL ||
                        adjacent == Types.BUILDING.FORGE) {
                    int adjacentCityId = board.getCityIdAt(adjPos.x, adjPos.y);
                    if (adjacentCityId != -1 && tribe.controlsCity(adjacentCityId)) {
                        adjFound = true;
                        break;
                    }
                }
        ...
        if(checkIfUnique) {
            for (Building existingBuilding : city.getBuildings()) {
                if (existingBuilding != null && existingBuilding.type == buildingType) {
                    return false;
                }
            }
        }
