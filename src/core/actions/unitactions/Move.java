...
public class Move extends UnitAction
{
    ...
    public boolean isFeasible(final GameState gs)
    {
        ...
        if(unit == null || destination == null)
            return false;
        if (!gs.getTribe(unit.getTribeId()).isExplored(destination.x, destination.y))
            return false;
        ...
        if(unit.canMove() && gs.getBoard().getObservableUnitAt(unit.getTribeId(), destination.x, destination.y) == null) {
