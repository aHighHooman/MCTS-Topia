...
public class DisbandCommand implements ActionCommand {
    ...
    public boolean execute(Action a, GameState gs) {
        ...
        {
            int disbandCost = unit.COST;
            if (unit instanceof CarrierUnit) {
                Types.UNIT baseLandUnit = ((CarrierUnit) unit).getBaseLandUnit();
                if (baseLandUnit != null) {
                    disbandCost = baseLandUnit.getCost();
                }
            }
            int starsGained = (int) (disbandCost / 2.0); //half, rounded down
