...
public class StepMove implements NeighbourHelper
{
    ...
    public ArrayList<PathNode> getNeighbours(Vector2d from, double costFrom) {
        ...
        boolean onRoad = board.canUseRoad(unit.getTribeId(), from.x, from.y);
        boolean creepUnit = unit.getType() == Types.UNIT.CLOAK;
        ...
        for(Vector2d tile : from.neighborhood(1, 0, board.getSize())) {
            ...
            Unit otherUnit = board.getObservableUnitAt(unit.getTribeId(), tile.x, tile.y);
            ...
            if (!creepUnit) {
                for (Vector2d tileAdj : tile.neighborhood(1, 0, board.getSize())) {
                    Unit u = board.getObservableUnitAt(unit.getTribeId(), tileAdj.x, tileAdj.y);  // There might not be a unit there at all
            ...
            //Movement is legal onto explored tiles that are not still under clouded fog.
            if (!gs.getTribe(unit.getTribeId()).isExplored(tile.x, tile.y)) {
                continue;
            }
            ...
            } else //Ground unit
            {
                switch (terrain) {
                    case SHALLOW_WATER:
                    case DEEP_WATER:
                        if (board.isBridge(tile.x, tile.y)) {
                            stepCost = 1.0;
                        }
                        //Embarking takes a turn of movement.
                        else if (board.getBuildingAt(tile.x, tile.y) == Types.BUILDING.PORT &&
                                board.canUsePort(unit.getTribeId(), tile.x, tile.y)) {
                            stepCost = costFrom < unit.MOV ? (unit.MOV - costFrom) : unit.MOV; //as much cost as needed to finished step here;
                        } else {
                            continue;
                        }
                ...
                if (!creepUnit && onRoad && board.canUseRoad(unit.getTribeId(), tile.x, tile.y)) {
