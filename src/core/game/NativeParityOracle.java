...
public final class NativeParityOracle {
    ...
    public static void main(String[] args) {
        try {
            ...
            out.put("children", stepAllRootActions(session, actions, actionIds));
    ...
    private static JSONArray stepAllRootActions(ExternalForwardModelSession session,
                                                ArrayList<Action> actions,
                                                ArrayList<String> actionIds) {
        JSONArray children = new JSONArray();
        for (int i = 0; i < actions.size(); i++) {
            JSONObject child = new JSONObject();
            child.put("source_state_id", ExternalBotPayloadBuilder.ROOT_STATE_ID);
            child.put("action_index", i);
            child.put("action_id", actionIds.get(i));
            try {
                JSONObject request = new JSONObject();
                request.put("command", "step");
                request.put("state_id", ExternalBotPayloadBuilder.ROOT_STATE_ID);
                request.put("action_id", actionIds.get(i));
                JSONObject response = session.handleCommand(request);
