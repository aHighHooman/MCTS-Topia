            ...
            for(int j = 0; j < tile.length; ++j)
            {
                ...
                if(terrainChar == Types.TERRAIN.CITY.getMapChar())
                {
                    ...
                    if (tileSplit.length >= 3) {
                        tribeID = Integer.parseInt(tileSplit[2]);
                    } else {
                        for(Tribe t : tribes)
                        {
    ...
    private Tribe[] extractTribes(String[] lines)
    {
        ...
        for (int i = 0; i < size.height; ++i) {
            ...
            for (String s : tile) {
                ...
                if (isCity) {
                    int tribeType = Integer.parseInt(tileSplit[1]);
                    Types.TRIBE tribe = Types.TRIBE.getTypeByKey(tribeType);

                    if (tileSplit.length >= 3) {
                        int tribeSlot = Integer.parseInt(tileSplit[2]);
                        while (tribes_list.size() <= tribeSlot) {
                            tribes_list.add(null);
                        }
                        tribes_list.set(tribeSlot, tribe);
                    } else {
                        tribes_list.add(tribe);
                    }
                }
            }
        }

        ArrayList<Types.TRIBE> orderedTribes = new ArrayList<>();
        for (Types.TRIBE tribeType : tribes_list) {
            if (tribeType != null) {
                orderedTribes.add(tribeType);
            }
        }

        Tribe[] tribesArray = new Tribe[orderedTribes.size()];
        int i = 0;
        for(Types.TRIBE trType : orderedTribes)
        {
