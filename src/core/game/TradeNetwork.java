        ...
        if (tribe.controlsCapital()) {
            ...
            for (int i = 0; i < networkTiles.length; ++i) {
                for (int j = 0; j < networkTiles[0].length; ++j) {
                    ...
                    if (board.canUseRoad(tribe.getTribeId(), i, j)) {
                        connectedTiles[i][j] = networkTiles[i][j];
                    }

                    if (board.canUsePort(tribe.getTribeId(), i, j)) {
                        connectedTiles[i][j] = networkTiles[i][j];
                        ports.add(new Vector2d(i, j));
                    }

                    boolean allowedWaterTerritory = cityId == -1;
                    if (!allowedWaterTerritory) {
                        City territoryCity = (City) board.getActor(cityId);
                        if (territoryCity != null) {
                            int ownerTribeId = territoryCity.getTribeId();
                            allowedWaterTerritory = ownerTribeId == tribe.getTribeId()
                                    || board.getDiplomacy().isTreaty(tribe.getTribeId(), ownerTribeId);
                        }
                    }
                    if (ter == SHALLOW_WATER && allowedWaterTerritory) {
                        navigable[i][j] = true;
                    } else if (ter == DEEP_WATER && allowedWaterTerritory
                            && tribe.getTechTree().isResearched(Types.TECHNOLOGY.NAVIGATION)) {
                        navigable[i][j] = true;
                    }
                }
            }
